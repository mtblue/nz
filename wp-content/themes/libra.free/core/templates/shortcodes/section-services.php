...
$ss_span_class = "span". ($sidebar_layout != 'sidebar-no' ? 2 : 2);   // che cosa è questa riga?? 
...
if( $services->have_posts() ) :
    ...
    ?>
	<!-- <div class="row">  il row non serve messo qui, subito dopo ci deve essere uno span, ma non ci stanno! -->
				...
				<div class="<?php echo $ss_span_class ?>">
					<div class="related_project">
						<a title="<?php if($show_title_hover == 'yes' || $show_title_hover == '1'): ?><?php the_title() ?><?php endif ?>" href="<?php the_permalink() ?>" class="related_img<?php if($show_detail_hover == 'yes' || $show_detail_hover == '1' ): ?> related_detail<?php elseif($show_title_hover == 'yes' || $show_title_hover == '1' ): ?> related_title<?php endif ?>">
							<?php echo has_post_thumbnail() ? get_the_post_thumbnail( get_the_ID(), 'thumb_portfolio_fulldesc_related' ) : '<img src="' . YIT_CORE_ASSETS_URL . '/images/no-featured-175.jpg" title="' . __( '(this post does not have a featured image)', 'yit' ) . '" alt="" />' ?>
						</a>

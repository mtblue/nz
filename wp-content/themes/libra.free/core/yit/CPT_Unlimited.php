...
class YIT_CPT_Unlimited {     
    ...
    */
    ...
    protected function _settingsOption( $option, $item_id = 0 ) {
        ...
        ?><div class="the-metabox <?php echo $option['type'] ?> <?php echo $option['id'] ?> clearfix"><?php
...
//         return $c;
    ...
    protected function _queryVarsValidate() {
        ...
        foreach ( $items as $item_id => $the_ ) {
            $stop = sanitize_title( $the_['title'] ) == $item_name ? true : false;
            if ( $stop ) break;
        }
        if ( ! $stop )      
            { return false; }
    ...
    public function load_template_single( $template ) {
        ...
        if ( ! empty( $item_name ) ) {
            foreach ( $items as $item_id => $the_ ) {
                if ( sanitize_title( $the_['title'] ) == $item_name )
                    { break; }
            }     

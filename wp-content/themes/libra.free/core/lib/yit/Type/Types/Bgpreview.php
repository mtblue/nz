...
class YIT_Type_BgPreview {
	...
	public static function display( $value ) {
		...
		ob_start(); ?>
			<div id="<?php echo $value['id_container'] ?>" class="yit_options rm_option rm_input rm_select rm_bg-preview">
                <div class="option">
	                ...
	                <div class="clear"></div>
	                <div class="bg-preview" id="<?php echo $value['id'] ?>_preview"><div style="background: <?php echo $bg['color'] ?> url('<?php echo ( $bg['image'] != 'custom' ) ? $bg['image'] : '' ?>') 50% 0"></div></div>
            ...
            jQuery(document).ready( function( $ ) {
            	var preview = "#<?php echo $value['id'] ?>_preview div";
                if( $('#<?php echo $value['id'] ?>_image').val() == 'custom' || $('#<?php echo $value['id'] ?>_image').val() == '' ) {
                    $(preview).hide();
                }
				$('#<?php echo $value['id'] ?>_image').change(function(){
					if( $(this).val() == 'custom' || $('#<?php echo $value['id'] ?>_image').val() == '' ) {
						$(preview).hide();
					} else {
				...
				$('#<?php echo $value['id'] ?>_color_container').ColorPicker({
					color: '<?php echo $bg['color'] ?>',
					...
					onChange: function (hsb, hex, rgb) {
						$('#<?php echo $value['id'] ?>_color_container div').css('backgroundColor', '#' + hex);
						$( '#<?php echo $value['id'] ?>_color_container' ).next( 'input' ).attr( 'value', '#' + hex );

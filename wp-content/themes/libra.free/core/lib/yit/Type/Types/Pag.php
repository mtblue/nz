...
class YIT_Type_Pag {
	...
	public static function display( $value ) {
		...
		ob_start(); ?>
			<div id="<?php echo $value['id_container'] ?>" class="yit_options rm_option rm_input rm_multi_checkbox">
                ...
                <ul id="<?php echo( $value['id'] . $ext ); ?>" class="list-sortable<?php echo $class ?>">  
    			        ...
    			        $selected_pags = explode(",", yit_get_option($value['id'] . $ext));
                        
                        if($heads) echo '<li class="head">'.$value['heads'][$i-1].'</li>';
                        
                        $c = 0;
        				foreach($pags as $pag) { $checked = isset( $checked_items[$i] ) ? $checked_items[$i] : array(); ?>
                        
                        	<li>
			                	<label class="radio-inline">
									<input type="checkbox" name="<?php yit_field_name( $value['id'] ); ?>[<?php echo $i ?>][]" value="<?php echo $pag->ID; ?>" <?php checked( in_array( $pag->ID, $checked ), true ); ?> id="<?php echo( $value['id'] ); ?>-<?php echo $c . $ext ?>" />&nbsp;

...
$show_title      = $post_id != 0 && ( is_page() || is_posts_page() ) ? yit_get_post_meta( $post_id, '_show-title' ) : false;
$show_breadcrumb = $post_id != 0 && ( is_page() || is_posts_page() ) ? yit_get_post_meta( $post_id, '_show-breadcrumb' ) : ( bool ) yit_get_option('breadcrumb');
...
<div id="page-meta" class="group">
    ...
    <?php if ( $show_title ) : ?>
        ...
        <?php   
        	...
        	<?php if( yit_get_option( 'icon-title' ) ) : ?>
                <div class="icontitle">
                	<img src="<?php echo yit_get_option( 'icon-title' ) ?>" alt="title" />
                </div>
                <div class="title-with-icon"><<?php echo $tag_title ?>><?php echo yit_decode_title(get_the_title($post_id)) ?></<?php echo $tag_title ?>></div>
            <?php else : ?>
            	<<?php echo $tag_title ?>><?php echo yit_decode_title(get_the_title($post_id)) ?></<?php echo $tag_title ?>>
            <?php endif;             
        endif; ?>
        </div>
    <?php endif; ?>
    ...
    <?php if ( $show_breadcrumb ) : ?>
        ...
        <div class="breadcrumbs">
        	<?php if ( yit_get_option('breadcrumb-text') != '' ) : ?>
        		<span class="before-text"><?php echo yit_get_option('breadcrumb-text') ?></span>
        	<?php endif ?>
        <?php

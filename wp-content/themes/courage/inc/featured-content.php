...
class Courage_Featured_Content {
	 ...
	 *
	 * @uses Courage_Featured_Content::$max_posts
	 */
	public static function init() {

		// Get Number of Posts from Theme settings
		
		add_filter( 'courage_get_featured_content',       array( __CLASS__, 'get_featured_posts' )    );
	 ...
	 * Get featured posts
	 *
	 * @uses Courage_Featured_Content::get_featured_post_ids()
	 *
	 * @return array
	 */
	...
	public static function get_featured_post_ids() {
		...
		if ( ! empty( $featured_ids ) ) {
			return array_map( 'absint', apply_filters( 'courage_featured_content_post_ids', (array) $featured_ids ) );
		}
		...
		if ( $term ) {
			...
			return apply_filters( 'courage_featured_content_post_ids', array() );
		}
		...
		return apply_filters( 'courage_featured_content_post_ids', $featured_ids );
	}
	 ...
	 * @see Courage_Featured_Content::validate_settings().
	 */
	 ...
	 *
	 * @uses Courage_Featured_Content::get_featured_post_ids();
	 * @uses Courage_Featured_Content::get_setting();
	 ...
	 * is deleted by Courage_Featured_Content::validate_settings().
	 *
	 ...
	 *
	 * @uses Courage_Featured_Content::validate_settings()
	 *
	 * @return void
	 */
	...
	public static function customize_register( $wp_customize ) {
		$wp_customize->add_section( 'courage_featured_content', array(
			'title'          => __( 'Featured Content', 'courage' ),
			'description'    => sprintf( __( 'Easily feature all posts with the <a href="%1$s">"featured" tag</a> or a tag of your choice. Your theme supports up to %2$s posts in its featured content area.', 'courage' ), admin_url( '/edit.php?tag=featured' ), absint( self::$max_posts ) ),
			'priority'       => 40,
		 ...
		 *
		 * Sanitization callback registered in Courage_Featured_Content::validate_settings().
		...
		$wp_customize->add_control( 'featured-content[tag-name]', array(
			'label'          => __( 'Tag name', 'courage' ),
			'section'        => 'courage_featured_content',
			'priority'       => 20,
		) );
		$wp_customize->add_control( 'featured-content[hide-tag]', array(
			'label'          => __( 'Hide tag from displaying in post meta and tag clouds.', 'courage' ),
			'section'        => 'courage_featured_content',
			'type'           => 'checkbox',
			'priority'       => 30,
		) );
		$wp_customize->add_control( 'featured-content[show-all]', array(
			'label'          => __( 'Display featured posts in latest blog post listing.', 'courage' ),
			'section'        => 'courage_featured_content',
			'type'           => 'checkbox',
			'priority'       => 40,
		) );
	...
	public static function enqueue_scripts() {
		wp_enqueue_script( 'courage-featured-content-suggest', get_template_directory_uri() . '/js/featured-content-admin.js', array( 'jquery', 'suggest' ), '20150401', true );
	}
	 ...
	 * Courage_Featured_Content::get_featured_content().
	 *
	 * @uses Courage_Featured_Content::delete_transient()
	 *
	 * @param array $input
	 * @return array $output
	 */
...
Courage_Featured_Content::setup();
